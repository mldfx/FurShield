...
const HealthTimeline = () => {
  ...
  // Fetch health records from backend, fallback to localStorage
  useEffect(() => {
  ...
  fetch(`${API_BASE}/api/health-timeline`, {
      ...
      .catch(() => setRecords([...records, { ...form, id: Date.now() }]));
  ...
  <button className="back-dashboard-btn" onClick={() => navigate("/dashboard/pet-owner")}> 
        ⟵ Back to Dashboard
      </button>

      <div className="healthTimeline-card">
        <h2>
          <PawPrint /> Pet Health Timeline
        </h2>
        <p className="healthTimeline-intro">
          Visualize your pet's health journey. See Vaccinations, Treatments, Illnesses, Allergies, and Milestones in a
        ...
        <form onSubmit={handleSubmit} className="timeline-form">
          ...
          <button type="submit" className="timeline-add-btn">
            + Add Event
          </button>
        </form>
